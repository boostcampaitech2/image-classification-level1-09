...
class CustomAugmentation:
    def __init__(self, mean, std, **args):
        self.transform1 = Compose([
            CenterCrop(224, 224),
            # RandomContrast(limit=(-0.25, 0.75), p=0.5)
            # Solarize(threshold=0, p=0.6),
            # ToGray(p=1)
            RandomBrightnessContrast(contrast_limit=(0.8, 0.8), p=1),
            # GaussianBlur(blur_limit=(199, 199), p=0.6)
            # Resize(resize, Image.BILINEAR),
            # ToTensor(),
            # Normalize(mean=mean, std=std),
            # GaussNoise(var_limit=(150.0, 200.0), p=0.7)
        ])
        self.transform2 = tr.Compose([tr.ToTensor(),
                                      tr.Normalize(mean=mean, std=std)])

    def __call__(self, image):
        image = self.transform1(image=np.array(image))['image']
...
class MaskBaseDataset(Dataset):
    ...
    def __init__(self, data_dir, mean=(0.548, 0.504, 0.479), std=(0.237, 0.247, 0.246), val_ratio=0.2):
        ...
        self.transform = None
        self.cutmix = CutMix()
        self.cutmix_cnt = 0 # cutmix할 사람 수 count
        self.setup()
        self.calc_statistics()

        self.middle = len(list(filter(lambda x : 30 <= x < 60, self.age_labels))) # 중년층 수
    ...
    def __getitem__(self, index):
        ...
        image_transform = self.transform(image) # transfrom 결과
        
        # 데이터 불균형 맞추기 위한.. 나중에 다시 보고 주석 풀기
        # if age_label in list(range(1, 18, 3)) and self.cutmix_cnt < self.middle // 2:
        #     image_transform = self.cutmix(image_transform)
        #     self.cutmix_cnt += 1
...
class TestDataset(Dataset):
    def __init__(self, img_paths, mean=(0.548, 0.504, 0.479), std=(0.237, 0.247, 0.246)):
        ...
        self.transform = CustomAugmentation(mean, std)
