...
def train(data_dir, model_dir, args):
    ...
    scheduler = CosineAnnealingLR(optimizer, T_max=args.lr_decay_step, eta_min=0)
    ...
    for epoch in tqdm(range(args.epochs)):
        ...
        for idx, train_batch in enumerate(train_loader):
            inputs, labels, active_cutmixs = train_batch
            
            inputs = inputs.to(device)
            inputs = CutMix(inputs, active_cutmixs)().to(device)
